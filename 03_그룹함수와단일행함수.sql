-- 그룹함수와 단일행함수
-- 단일행 함수 (SINGLE ROW FUNTION): N개의 값을 넣어 N개의 결과를 리턴
-- 그룹함수 (GROUP FUNTION) : N개의 값을 넣어 1개의 결과를 리턴
...
-- 단일행 함수
-- 문자 관련 함수
-- : LENGTH, LENGTHB, SUBTR, UPPER, LOWER, INSTR...
SELECT
       LENGTH('오라클')
     , LENGTHB('오라클')
...
-- LTRIM / RTRIM : 주어진 컴럼이나 문자열 왼쪽/오른쪽에서 
...
-- REPLACE : 컴럼 혹은 문자열을 입력받아 변경하고자 하는 문자열을 변경하려고 하는 문자열로 바꾼 후 리턴
...
SELECT SYSDATE FROM DUAL; --SYSDATE 현재날짜
...
SELECT
       *
  FROM EMPLOYEE;
 -- WHERE ADD_MONTHS(HIRE_DATE, 240) <= SYSDATE; -- 첫번째 방법
...
SELECT TO_CHAR(123456, '99999') FROM DUAL;
...
SELECT TO_CAHR(SYSDATE, 'YEAR, Q') || '분기' FROM DUAL;
