-- 그룹함수와 단일행함수
-- 단일행 함수(SINGLE ROW FUCTION) : N개의 값을 넣어 N개의 결과를 리턴
...
-- EMPLOYEE 테이블에서 근무 년수가 20년 이상의 직원 조회
SELECT
       *
  FROM EMPLOYEE
...
-- 문자는 날짜로 자동 형변환 되지만, 숫자는 날짜로 자동 형변환 안되
...
SELECT
     ...
     , SUBSTR(EMP_NO, 8) 뒷부분
     , TO_NUMBER(SUBSTR(EMP_NO, 1, 6)) + TO_NUMBER(SUBSTR(EMP_NO, 8)) 결괴
  FROM EMPLOYEE
 WHERE EMP_ID = '201';
